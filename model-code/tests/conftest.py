...
def mock_mlflow_run(monkeypatch):
    ...
    def mock_noop(*args, **kwargs):
        # Mock no-op : neutralise les appels MLflow pendant les tests.
        return None

    import mlflow

    monkeypatch.setattr(mlflow, "start_run", mock_start_run)
    monkeypatch.setattr(mlflow, "log_param", mock_noop)
    monkeypatch.setattr(mlflow, "log_params", mock_noop)
    monkeypatch.setattr(mlflow, "log_metric", mock_noop)
    monkeypatch.setattr(mlflow, "log_metrics", mock_noop)
    monkeypatch.setattr(mlflow, "log_artifact", mock_noop)
    monkeypatch.setattr(mlflow.sklearn, "log_model", mock_noop)
